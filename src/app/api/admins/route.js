    ...
    try {
        ...
        if (
            ...
            return NextResponse.json(
                { message: "Name, email and image file are all required" },
                { status: 400 }
            );
        }
        ...
        const newCategory = await adminSchema.create({ name, email, image: imageUrl });
...
export async function GET(req) {
    try {
        ...
        const categories = await adminSchema.find(); // सबै categories खोज्नुहोस्
