...
import collectionsSchema from "../../lib/models/collections/collectionsSchema";
...
export async function POST(req) {
    try {
        ...
        const newCategory = await collectionsSchema.create({ name, selectedProducts, subtitle, image: imageUrl });
...
export async function GET(req) {
    try {
        ...
        const categories = await collectionsSchema.find(); // सबै categories खोज्नुहोस्
...
export async function DELETE(req) {
  try {
      ...
      const response = await collectionsSchema.findByIdAndDelete(id);
